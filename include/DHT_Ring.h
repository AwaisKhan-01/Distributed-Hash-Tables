...
public:
    ...
    bool add_machine(BigInt& newHash, string Mname, string filename, int BTreeOrder)
    {
    ...
    int getExistingMachines() const {
        return Existing_machines;
    }

    int getBitsSpace() const {
        return bits_space;
    }
};
...
bool createDirectory(std::string dirname) {
    ...
    std::string fullPath = "./" + dirname;
    std::error_code ec;
    bool result = std::filesystem::create_directories(fullPath, ec);
    if (result) {
        std::cout << "Directory created successfully\n";
        return true;
    } else if (!ec) {
...
bool deleteDirectory(std::string dirname) {
    ...
    if (!ec) {
        ...
        std::cout << "Error deleting directory: " << ec.message() << "\n";
        return false;
    }
#endif
}
